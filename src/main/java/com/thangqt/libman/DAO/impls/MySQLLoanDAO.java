...
public class MySQLLoanDAO implements LoanDAO {
  ...
  public void add(Loan loan) throws SQLException {
    String query =
        "INSERT INTO loans (user_id, material_id, borrow_date, due_date) VALUES (?, ?, ?, ?)";
  ...
  public boolean isDocumentIssued(int userId, int materialId) throws SQLException {
    String query =
        "SELECT * FROM loans WHERE user_id = ? AND material_id = ? AND (return_date IS NULL OR return_date > CURDATE())";
    try (PreparedStatement stm = conn.prepareStatement(query)) {
      stm.setInt(1, userId);
      stm.setInt(2, materialId);
      ResultSet rs = stm.executeQuery();
  ...
  public List<Loan> getOverdueLoanWithinDayRange(int range) throws SQLException {
    String query =
        "SELECT * FROM loans WHERE due_date >= CURDATE() - INTERVAL ? DAY AND return_date IS NULL";
    try (PreparedStatement stm = conn.prepareStatement(query)) {
      stm.setInt(1, range);
      ResultSet rs = stm.executeQuery();
      List<Loan> loans = new ArrayList<>();
