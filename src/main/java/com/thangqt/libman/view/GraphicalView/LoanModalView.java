...
public class LoanModalView {
  ...
  private TextField borrowPeriodField;
  ...
  private Label createHeadline() {
    Label headline = new Label("Add Loan");
  ...
  private void createFormFields() {
    ...
    borrowPeriodField = new TextField();
    borrowPeriodField.setPromptText("Borrow Period (days)");
  ...
  private VBox createInfoContainer() {
    ...
    infoContainer.getChildren().addAll(borrowerInfoLabel, materialInfoLabel);
  ...
  private void validateAndProcessQRCode(String qrCode) {
    ...
    try {
      ...
      if (material != null && user != null && borrowPeriodField.getText().matches("\\d+")) {
  ...
  private void setupFormValidation() {
    ChangeListener<String> inputListener =
        (observable, oldValue, newValue) -> {
          ...
          boolean isPeriodValid = borrowPeriodField.getText().matches("\\d+");
          addLoanBtn.setDisable(!(isBorrowerValid && isMaterialValid && isPeriodValid));
        };

    borrowerIdField.textProperty().addListener(inputListener);
    materialIdField.textProperty().addListener(inputListener);
    borrowPeriodField.textProperty().addListener(inputListener);
  }
  ...
  private Alert createConfirmationAlert(User user, Material material) {
    ...
    confirmation.setTitle("Add Loan");
    confirmation.setHeaderText("Are you sure you want to add this loan?");
  ...
  private void createLoan(User user, Material material) {
    try {
      Loan loan =
          new Loan(
              user.getId(),
              material.getId(),
              LocalDate.now(),
              LocalDate.now().plusDays(Integer.parseInt(borrowPeriodField.getText())));
  ...
  private void showSuccessAlert() {
    ...
    alert.setHeaderText("Loan added successfully");
    alert.showAndWait();
  }
