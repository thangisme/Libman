...
public class HomeController {
  ...
  @FXML private Text totalMaterialsNum;

  @FXML private Text borrowedNum;

  @FXML private Text overdueNum;

  @FXML private Text totalUsersNum;

  @FXML private Card checkoutStatsCard;

  @FXML private VBox overdueTableContainer;

  @FXML private VBox recentlyBorrowedTableContainer;

  @FXML private VBox recentlyAddedMaterialsContainer;
  ...
  private LineChart<String, Number> createCheckoutStatsChart() throws SQLException {
    ...
    borrowedCount.forEach(
        (date, count) -> borrowedNum.getData().add(new XYChart.Data<>(date, count)));
    returnedCount.forEach(
        (date, count) -> returnedNum.getData().add(new XYChart.Data<>(date, count)));
  ...
  private TableView<Loan> createOverdueTable() throws SQLException {
        ...
        c -> {
          try {
            return new SimpleStringProperty(
                materialManager.getMaterialById(c.getValue().getMaterialId()).getTitle());
    ...
    col2.setCellValueFactory(
        c -> {
          try {
            return new SimpleStringProperty(
                materialManager.getMaterialById(c.getValue().getMaterialId()).getAuthor());
          } catch (SQLException e) {
            e.printStackTrace();
            return new SimpleStringProperty("Error");
          }
    ...
    col3.setCellValueFactory(c -> new SimpleStringProperty(c.getValue().getDueDate().toString()));
    ...
    col4.setCellValueFactory(
        c -> {
          try {
            return new SimpleStringProperty(
                userManager.getUserById(c.getValue().getUserId()).getName()
                    + " (#"
                    + c.getValue().getUserId()
                    + ")");
          } catch (SQLException e) {
  ...
  private TableView<Loan> createRecentlyBorrowedMaterials() throws SQLException {
    ...
    col4.setCellValueFactory(c -> new SimpleStringProperty(c.getValue().getDueDate().toString()));
